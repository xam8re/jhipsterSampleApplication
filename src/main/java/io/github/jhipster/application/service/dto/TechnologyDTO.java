package io.github.jhipster.application.service.dto;

import javax.validation.constraints.*;
import java.io.Serializable;
import java.util.Objects;

/**
 * A DTO for the Technology entity.
 */
public class TechnologyDTO implements Serializable {

    private Long id;

    @NotNull
    private String name;

    private Long technologyClassId;

    private Long machineModelId;

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public Long getTechnologyClassId() {
        return technologyClassId;
    }

    public void setTechnologyClassId(Long technologyClassId) {
        this.technologyClassId = technologyClassId;
    }

    public Long getMachineModelId() {
        return machineModelId;
    }

    public void setMachineModelId(Long machineModelId) {
        this.machineModelId = machineModelId;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }

        TechnologyDTO technologyDTO = (TechnologyDTO) o;
        if (technologyDTO.getId() == null || getId() == null) {
            return false;
        }
        return Objects.equals(getId(), technologyDTO.getId());
    }

    @Override
    public int hashCode() {
        return Objects.hashCode(getId());
    }

    @Override
    public String toString() {
        return "TechnologyDTO{" +
            "id=" + getId() +
            ", name='" + getName() + "'" +
            ", technologyClass=" + getTechnologyClassId() +
            ", machineModel=" + getMachineModelId() +
            "}";
    }
}
